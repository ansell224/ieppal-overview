...
The single source of truth for how IEP Pal looks, sounds, and feels.
...
- **Primary mark:** `Wesbite/logo-new.svg` — a smiling "o" mark, black stroke. Minimalist, friendly.
- **Favicon:** `Wesbite/favicon.svg`
...
The `Assets/` folder contains the IEP Pal illustration system:
...
The full voice guide lives in `brand/voice/SKILL.md`. In one paragraph:
...
├── README.md                    ← how to use this repo with Claude
├── brand/                       ← brand identity assets
│   ├── voice/SKILL.md           ← voice & tone rules (SHIPPED) — load before any copy
...
│   ├── carousel/SKILL.md        ← LinkedIn carousel skill (stub)
...
├── IEPPAL-RASA-FE-mvp/          ← production React frontend
└── Wesbite/                     ← live website source
```
